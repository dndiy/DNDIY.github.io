  ...
  onMount(async () => {
    try {
      ...
      if (!slug.startsWith('friend-')) {
        error = 'Invalid friend content slug format';
        loading = false;
        return;
      }
      ...
      console.log('Loading friend post with ID:', friendPostId);
      
      // Get all friend content
      const allFriendContent = getFriendContent();
      console.log('Retrieved friend content:', allFriendContent.length, 'posts');
      
      // Find the specific post
      const post = allFriendContent.find(p => p.slug === friendPostId || p.id === friendPostId);
      
      if (post) {
        console.log('Found matching friend post:', post);
        friendPost = post;
      } else {
        console.error('Friend post not found for ID:', friendPostId);
...
{:else if friendPost}
  <article>
    ...
    <!-- Friend Post Tags -->
    {#if friendPost.tags && friendPost.tags.length > 0}
      <div class="mt-6 flex flex-wrap gap-2">
        {#each friendPost.tags as tag}
          <span class="px-3 py-1 text-sm bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-300 rounded-full">
            {tag}
          </span>
        {/each}
      </div>
